...
class PDFJSExpressViewer extends Component {
    ...
    createStamped = async () => {
        ...
        const response = await Axios.get(`${CONSTANTS.ENDPOINT.USER}/ds-stamp`, { headers: { authorization: 'Bearer ' + this.context.user.token } });
        if (response.data) stampAnnot.ImageData = response.data.stamp;
                    ...
                    this.state.isLoaded ?
                        <Aux>
                            ...
                            <Button
                                className={classes.button}
                                variant="contained"
                                onClick={() => this.createStamped()}>Create QRCode Stamp
                            </Button>
                        </Aux>
                        : null}
